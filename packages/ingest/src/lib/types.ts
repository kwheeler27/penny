...
/** Mirrors @buck/db's `period_type` enum. Duplicated here (rather than imported) because @buck/db is a runtime dependency we want ingest's pure transform functions to stay decoupled from — see raw-observation.ts. Kept in sync by packages/db/test/db.test.ts and packages/ingest/test/reconciliation.test.ts both exercising the same enum values end to end. */
 ...
 * `@buck/db`'s `NewObservation` is the same shape with a couple of
