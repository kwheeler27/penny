# CBO baseline revenues projection — source and extraction
...
Category," row "Total" under the "Revenues" block, the dollar-amount block
...
`../baseline_outlays/2026-02-baseline-outlays.csv` — all three series are
...
CBO projection — already covered by `fiscal.mts.receipts.total`). Values
are copied verbatim from the workbook cells (3 decimal places as
published), sign preserved (revenues are always positive in this
baseline).

## Reconciliation check (required by the task that added this file)

For every fiscal year 2026–2036, this file's `total_revenues_usd_billions`
minus `../baseline_outlays/2026-02-baseline-outlays.csv`'s
`total_outlays_usd_billions` equals
